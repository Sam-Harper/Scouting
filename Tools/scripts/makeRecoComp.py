...
ELE_BRANCHES = [
    ...
    "recoEleIndx","transparencyCorr"
]
# reco (miniAOD) electron branches copied over for the matched electron; they
# end up in vals/the output tree as recoEle_<name>, 0 for unmatched electrons
RECO_ELE_BRANCHES = ["ecalEnergy", "ecalEnergyError"]
BRANCHES = (
    ["run", "luminosityBlock", "event", "ScoutingRho_fixedGridRhoFastjetAll"]
    + [f"ScoutingElectron_{name}" for name in ELE_BRANCHES]
    + ["nElectron"]
    + [f"Electron_{name}" for name in RECO_ELE_BRANCHES]
)

MASS_BINS = np.linspace(0.0, 120.0, 12001)
RATIO_BINS = np.linspace(0.0,2.0,100)


def get_reco_match_mask(events, indx):
    """
    Which scouting electrons have a usable index into the Electron table.
    """
    return (indx >= 0) & (indx < events["nElectron"])


def get_matched_reco(events, match, indx, name):
    """Flat per-scouting-electron values of the reco branch Electron_<name>.

    Electrons without a usable match get 0.  The gather has to happen here,
    while the arrays still have their per-event structure, as the index is
    an index into that event's reco electron collection.
    """
    # pad so that index 0 exists even in events with no reco electrons
    reco = ak.fill_none(ak.pad_none(events[f"Electron_{name}"], 1), 0.0)
    matched = reco[ak.where(match, indx, 0)]
    return ak.to_numpy(ak.flatten(ak.where(match, matched, 0.0)))
...
def get_flat_electrons(events):
    ...
    indx = events["ScoutingElectron_recoEleIndx"]
    match = get_reco_match_mask(events, indx)
    vals["recoEleIndx"] = np.where(ak.to_numpy(ak.flatten(match)), vals["recoEleIndx"], -1)
    for name in RECO_ELE_BRANCHES:
        vals[f"recoEle_{name}"] = get_matched_reco(events, match, indx, name)
    return vals, counts
...
def fill_ele_hists(corr_energies,vals, counts, id_mask, ele_hists):
    """Fill the ECAL-corrected / reco ecalEnergy ratio histograms.

    Only ID-passing electrons with a matched reco electron are used, as
    the unmatched ones have recoEle_ecalEnergy == 0.
    """
    matched = id_mask & (vals["recoEleIndx"] >= 0) & (vals["recoEle_ecalEnergy"] > 0)
    sel = ak.unflatten(matched, counts)

    def selected(flat_arr):
        return ak.unflatten(flat_arr, counts)[sel]

    hlt_to_reco = selected(corr_energies["corrEcalEnergy"])/selected(vals["recoEle_ecalEnergy"])

    hltlaser_to_reco = selected(corr_energies["corrEcalEnergy"])*selected(vals["transparencyCorr"])/selected(vals["recoEle_ecalEnergy"])
    is_eb = selected(np.abs(vals["eta"]) < 1.479)

    for region, region_mask in (("EB", is_eb), ("EE", ~is_eb)):
        ele_hists["hltToRecoRatioHist" + region] += np.histogram(
            ak.to_numpy(ak.flatten(hlt_to_reco[region_mask])),
            bins=RATIO_BINS
        )[0]
        ele_hists["hltLaserCorrToRecoRatioHist" + region] += np.histogram(
            ak.to_numpy(ak.flatten(hltlaser_to_reco[region_mask])),
            bins=RATIO_BINS
        )[0]
...
if __name__ == "__main__":
    ...
    ele_hists = {
        "hltToRecoRatioHistEB" : np.zeros(len(RATIO_BINS) - 1),
        "hltToRecoRatioHistEE" : np.zeros(len(RATIO_BINS) - 1),
        "hltLaserCorrToRecoRatioHistEB" : np.zeros(len(RATIO_BINS) - 1),
        "hltLaserCorrToRecoRatioHistEE" : np.zeros(len(RATIO_BINS) - 1)
    }
    ...
    for events in uproot.iterate(input_trees, BRANCHES, step_size=args.step_size,
        ...
        fill_ele_hists(corr,vals,counts,id_mask,ele_hists)

        out_branches = {name: arr for name, arr in corr.items()}
        out_branches.update(
            {f"recoEle_{name}": vals[f"recoEle_{name}"] for name in RECO_ELE_BRANCHES}
        )
        out_electrons = ak.zip(
            {name: ak.unflatten(arr.astype(np.float32), counts) for name, arr in out_branches.items()}
        )
        out_electrons["recoEleIndx"] = ak.unflatten(vals["recoEleIndx"], counts)
        out_chunk = {
            "run": events["run"],
    ...
    for name, counts_hist in ele_hists.items():
        output_file[name] = (counts_hist, RATIO_BINS)
    output_file.close()
